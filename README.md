...
before each. `--parallel` (or `exec.parallel` in the config) makes them run the repositories at once
with output captured, printing the header only for repositories that produced output. For the
parallel runs `--unordered` prints results as they finish
