...
func TestForcePushBranch(t *testing.T) {
	tests := []struct {
		...
		{
			name: "pushes to configured remote for branch",
			setup: func(t *testing.T, repoPath string) {
				...
				testutil.ExecOrFail(t, `
					...
					git remote add custom-remote `+remoteDir+`
					git config branch.feature.remote custom-remote
				`)
			},
			branchName:  "feature",
		...
		{
			name: "returns error when branch has no remote configured",
			setup: func(t *testing.T, repoPath string) {
				testutil.ExecOrFail(t, `
					git init
					git config user.email "test@example.com"
		...
		},
		{
			...
			expectError:   true,
			errorContains: "no remote configured for branch nonexistent",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			testutil.WithTempWorkingDir(t, func() {
				...
				repo := WithRepo(repoPath)
				err = repo.ForcePushBranch(tt.branchName)
