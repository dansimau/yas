...
func TestStatus_FromSubdirectoryInPathOrder(t *testing.T) {
	...
	result := newCLI(t, filepath.Join(target, "plain", "nested")).Run("st", "--header")
...
func TestStatus_ShowsChangesPerRepository(t *testing.T) {
	...
	result := newCLI(t, target).Run("status", "--header")
	assert.Equal(t, result.ExitCode(), 0, result.Stderr())
	...
	// --quiet drops the headers of repositories with no changes.
	result = newCLI(t, target).Run("st", "--header", "--quiet", "--", "--short")
	assert.Equal(t, result.ExitCode(), 0, result.Stderr())
...
func TestStatus_ArgumentsReplaceDefaults(t *testing.T) {
	...
	result := newCLI(t, target).Run("st", "--header", "--long")
	assert.Equal(t, result.ExitCode(), 0, result.Stderr())
...
func TestStatus_Unordered(t *testing.T) {
	...
	result := newCLI(t, target).Run("--unordered", "st", "--header")
	assert.Equal(t, result.ExitCode(), 0, result.Stderr())
	headerIndexes(t, result.Stdout(), "feature")
}

func TestStatus_HeadersOnlyOnTerminalOrWithHeaderFlag(t *testing.T) {
	t.Parallel()
	...
	assert.Equal(t, result.ExitCode(), 0, result.Stderr())
	assert.Assert(t, !strings.Contains(result.Stdout(), "==>"), result.Stdout())
...
func TestStatus_OutsideWorkyard(t *testing.T) {
	...
	result = newCLI(t, outside, "WORKYARD_ROOT", target).Run("st", "--header")
	assert.Equal(t, result.ExitCode(), 0, result.Stderr())
	headerIndexes(t, result.Stdout(), "feature")
}
...
func TestStatus_MissingRepositoryIsReportedAndSkipped(t *testing.T) {
	...
	result := newCLI(t, target).Run("st", "--header")
	assert.Equal(t, result.ExitCode(), 0, result.Stderr())
