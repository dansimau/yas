...
- **pkg/workyardcli**: go-flags CLI for workyard (`create`, `git`, `exec`, `list`/`ls`, `switch`/`sw`, `hook`, `remove`)
...
- **pkg/shellhook**: Shell hook mechanism used by yas and workyard (`shellhook.Hook{Command, EnvVar}`): `<cmd> hook bash|zsh` prints a `<cmd>()` shell function that sources the commands (e.g. `cd`) the binary appends to the file named by the env var (`YAS_SHELL_EXEC` / `WORKYARD_SHELL_EXEC`)
- **pkg/cliutil**: Prompts and the interactive arrow-key selector (`InteractiveSelect`) used by `yas branch` and `workyard switch`
...
- `pkg/workyard.PlanCreate` resolves the destination (`--dest`, else `<yards-dir>/<name>` with `yards-dir` from `config.yaml`, default `.workyard/yards`; the name must be `filepath.IsLocal` and is the default branch), guards (destination empty, no containment except a target under the source's `.workyard/` that does not swallow `.workyard/yards`; the source must not be or be inside a git repository, nor inside a workyard), scans the source depth-first with at most `NumCPU` goroutines (never following symlinks, never descending into repos; a dir is a repo if it has a `.git` entry or a bare layout) and resolves the branch per repo: existing local branch (error if checked out) → remote-only branch (tracking) → tag/commit (detached) → create from trunk (`config.yaml` `trunk`/`repos.<path>.trunk`, else `Repo.DetectMainBranch`). Repos sharing a git common dir get the branch once; the rest are detached
- `Create` writes the metadata (`complete: false`) and pointer first, copies non-repo subtrees (APFS `clonefile` on macOS, plain copy otherwise), adds worktrees with hooks disabled (serialized per common dir), restores ancestor directory modes, then marks the metadata complete. Every step registers an undo in a `rollback` list (worktree + created branch, metadata, destination including any ancestors it had to create); on failure they run in reverse and any undo failures are reported together
- `git`/`exec` fan out a command over every repo in the metadata (working directory = the repo); unknown options and everything after the first positional are passed to the command verbatim; any repo where the command failed or could not run makes the exit code 1. Two modes: captured (`Yard.Run`: all repos at once, output buffered, header only for repos with output) and serial (`Yard.RunSerial`: one repo at a time with stdin/stdout/stderr connected to the terminal so commands can be interactive and colored, header always printed). The command inherits the environment untouched (no `CleanedGitEnv`, no color flags) so it behaves exactly as if the user ran it in that repo. `git` is `exec git` with the arguments verbatim; both are serial unless `--parallel` or `exec.parallel: true` in the source's `config.yaml` (`--no-parallel` overrides). Root discovery: `WORKYARD_ROOT`, else walk up to a `.workyard` file. Source discovery (`create` without `--source`, `list`): the current yard's source, else the nearest ancestor with a `.workyard` directory, else the cwd (`workyard.FindSource`)
- `switch` shows the `list` lines in `cliutil.InteractiveSelect` (cursor on the current yard) and writes `cd <target>` for the workyard shell hook (`workyard hook bash|zsh`, env `WORKYARD_SHELL_EXEC`, separate from yas's so `workyard exec yas ...` cannot write to it); it refuses to start without the hook
- `remove` takes a name (`workyard.FindNamed`: `<yards-dir>/<name>` of the `FindSource` source) or a path (any argument that exists), else the current yard; it removes each worktree through its source repo (refusing dirty ones without `-f`), deletes the directory (and parents left empty up to the yards dir), prunes, deletes the branches workyard created (only fully merged ones unless forced) and the metadata. A yard whose source is gone is deleted outright (`RemoveOrphan`)
